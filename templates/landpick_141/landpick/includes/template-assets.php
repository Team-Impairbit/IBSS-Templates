...
function landpick_styles() {
	...
	wp_enqueue_style( 'animate', LANDPICK_URI. '/css/animate.css', false, '1.0.0.1' );	
	...
	wp_enqueue_style( 'landpick', LANDPICK_URI. '/css/landpick.css', false, '1.0.0' );
	if( function_exists('is_woocommerce') ){
    	wp_enqueue_style( 'landpick-woocommerce', get_theme_file_uri( '/css/woocommerce.css' ), array('woocommerce-general'), '1.0.1.4' );
	}

	wp_enqueue_style( 'landpick-default-style', LANDPICK_URI. '/css/style.css', false, LANDPICK_VERSION );
	wp_enqueue_style( 'landpick-style', get_stylesheet_uri(), false, LANDPICK_VERSION );
	if( is_rtl() ){
		wp_enqueue_style( 'landpick-styles-rtl', LANDPICK_URI. '/rtl/style-rtl.css', array('landpick-style'), LANDPICK_VERSION );		
	}
	
	wp_enqueue_style( 'landpick-responsive', LANDPICK_URI. '/css/responsive.css', array('landpick-style'), LANDPICK_VERSION );

	

	$landpick_layout_style = landpick_get_option( 'landpick_layout_style', 'rounded' );
	if( $landpick_layout_style != 'semirounded' ){
		wp_enqueue_style( 'landpick-layout-'.$landpick_layout_style, LANDPICK_URI. '/css/landpick-'.$landpick_layout_style.'.css', array('landpick-style'), LANDPICK_VERSION );
	}
}
...
function landpick_scripts() {
	...
	wp_enqueue_script( 'landpick-custom', get_theme_file_uri(  '/js/custom.js' ), array( 'jquery', 'jquery-masonry', 'masonry' ), LANDPICK_VERSION, true );
	...
	$arr = array( 
		...
		'backtotop' => landpick_get_option( 'backtotop', 'on' )
		);
...
function landpick_widgets_block_editor() {
    remove_theme_support( 'widgets-block-editor' );
}
add_action( 'after_setup_theme', 'landpick_widgets_block_editor' );