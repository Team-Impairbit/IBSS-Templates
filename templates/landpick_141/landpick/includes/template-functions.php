...
function landpick_social_share( $icon = true, $args = array()) {
	...
	if( function_exists('perch_modules_social_share') ){
	  echo perch_modules_social_share($icon, $args);		
	}
...
if ( ! function_exists( 'landpick_get_trim_words' ) ) :
...
add_filter( 'perch_modules/newsletter_form/args', 'landpick_newsletter_form_args' );
function landpick_newsletter_form_args(){
    return array(
            ...
            'name_label' => esc_attr__( 'Name', 'perch' ),
            'name_placeholder' => '',
            'name_required' => false,           
            'name_field_before' => '',
            'name_field_after' => '',
            ...
            'email_label' => esc_attr__( 'Email *', 'perch' ),
            'email_placeholder' => '',
            'email_required' => true,
            'email_required_sign' => '*',
            'email_field_before' => '',
            ...
            // list
            'show_list' => false,
            'list_ids' => landpick_get_option('list_ids', array('Nextapp')),
            'columns' => false,
            'selected_lists' => landpick_get_option('selected_lists', array('Nextapp')),
...
function landpick_get_post_title_tag(){
    $tag = landpick_get_option('single_post_title_tag', 'h4');
    $tag = ( $tag != '' )? $tag : 'h4';
    return esc_attr($tag);
}

function landpick_post_title_before(){
    $tag = landpick_get_post_title_tag();
    $tag_size = landpick_get_option('single_post_title_tag_size', 'md'); 
    $tag_size = ($tag_size != '')?  $tag.'-'.$tag_size : '';

    return sprintf('<%s class="%s">', $tag, $tag_size );
}
function landpick_post_title_after(){
    $tag = landpick_get_post_title_tag();
    return sprintf('</%s>', $tag);
}

/*Portfolio Single Posts Tag Size*/

function landpick_get_single_portfolio_title_tag(){
    $tag = landpick_get_option('portfolio_single_post_title_tag', 'h3');
    $tag = ( $tag != '' )? $tag : 'h3';
    return esc_attr($tag);
}

function landpick_single_portfolio_title_before(){
    $tag = landpick_get_single_portfolio_title_tag();
    $tag_size = landpick_get_option('portfolio_single_post_title_tag_size', 'xs');
    $tag_size = ($tag_size != '')?  $tag.'-'.$tag_size : '';

    return sprintf('<%s class="%s mb-30">', $tag, $tag_size );
}
function landpick_single_portfolio_title_after(){
    $tag = landpick_get_single_portfolio_title_tag();
    return sprintf('</%s>', $tag);
}

/*related product Single Posts Tag Size*/

function landpick_get_single_related_product_title_tag(){
    $tag = landpick_get_option('related_product_single_post_title_tag', 'h3');
    $tag = ( $tag != '' )? $tag : 'h3';
    return esc_attr($tag);
}

function landpick_single_related_product_title_before(){
    $tag = landpick_get_single_related_product_title_tag();
    $tag_size = landpick_get_option('related_product_single_post_title_tag_size', 'xs');
    $tag_size = ($tag_size != '')?  $tag.'-'.$tag_size : '';

    return sprintf('<%s class="%s product_title entry-title mb-15">', $tag, $tag_size );
}
function landpick_single_related_product_title_after(){
    $tag = landpick_get_single_related_product_title_tag();
    return sprintf('</%s>', $tag);
}
