<?php
function landpick_newsletter_list_options(){
    if( function_exists('ES') ){
        $lists_id_name_map = ES()->lists_db->get_list_id_name_map();
    }else{
        $lists_id_name_map = array( '1' => esc_attr__('Default', 'appset') );
    }
    
    return $lists_id_name_map;   
}
...
function landpick_footer_newsletter_options( $metabox = false ){
    $options = array(        
        ...
        array(
             'id' => 'newsletter_list',
            'title' => esc_attr__( 'Newsletter list', 'landpick' ),        
            'default' =>  '1',
            'type' => 'select',
            'options' => landpick_newsletter_list_options(),
            'required' => array('newsletter_area', '=', true),
        ),
        array(
