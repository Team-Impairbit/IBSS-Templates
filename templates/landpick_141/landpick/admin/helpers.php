...
function landpick_validate_video_url($url = '#'){
  $url_parse = wp_parse_url($url);
  $url = str_replace('watch?v=', 'embed/', $url);
  return $url;
}

function landpick_filter_by_key_value ($array, $index, $value, $newarray = array()){
...
function landpick_import_json_file_body($option_name, $url ){
  ...
  if(add_option( $option_name, $body, true )){
      return $option_name. ' - ' . esc_attr__( 'Imported successfully', 'landpick' );
  }else{
    return false;
  }
}

add_action( 'wp_ajax_landpick_import_json', 'landpick_import_json_callback' );
function landpick_import_json_callback(){
  $url = esc_url($_POST['file']);
  $option_name = esc_attr($_POST['option_name']);
  
  echo landpick_import_json_file_body($option_name, $url);
  
  wp_die();
}

function landpick_find_rules_from_css($css = ''){
    $rules = array();
...
function landpick_font_icon_options($css_file, $prefix = '', $substr = 0){
  $options = array();
...
function landpick_image_icon_options($path = '', $substr = 0){
  $options = array();
  ...
  foreach ( glob( get_template_directory() . "/{$path}*.jpg" ) as $filename ) {
      if( file_exists($filename) ){
          $key = str_replace(get_template_directory() . "/{$path}", '', $filename);
          $options[$key] = $key;
      }    
...
function landpick_css_file_class_options($css_file, $prefix = '', $postfix = '', $substr = 0){
  $options = array();
  $file_path = get_template_directory(). '/' . $css_file;
  $file_url = get_template_directory_uri(). '/' . $css_file;
    if( file_exists($file_path) ){
      ...
      $rules = landpick_find_rules_from_css($subject);
...
*/
function landpick_content_search_replace_content($content, $search, $replace){
...
function landpick_set_clean_content_by_post_type( $post_type = 'page', $args = array() ){
    $output = array();
    if( $post_type == '' ) return;
    $default = array(
      'search' => '',
    ...
    if($search != '' && $replace != ''){
        ...
        foreach ( $post_list as $post ) {
            $my_post = array(
              'ID'           => $post->ID,
              'post_content' => landpick_content_search_replace_content($post->post_content, $search,  $replace),
            );
...
add_action( 'wp_ajax_landpick_content_search_replace', 'landpick_content_search_replace_callback' );
function landpick_content_search_replace_callback(){
  $results = '';
  $ids = $_POST['ids'];
  $search = $_POST['search'];
  ...
  foreach ($ids as $post_id) {
      ...
      $content = $post->post_content;
        ...
        $my_post = array(
            'ID'           => $post_id,
            'post_content' => landpick_content_search_replace_content($content, $search,  $replace),
        );     

      wp_update_post( $my_post );
      $results .= '<p>'.sprintf(esc_attr__( '%s items replace successfully for %s', 'landpick' ), $total_founds, $post->post_title).'</p>';
     

  }

 
  echo '<div id="message" class="updated landpick-message">';
