...
$default_alt = apply_filters( 'perch_vc_single_image_alt', __( 'Landpick image', 'landpick' ) );
...
switch ( $source ) {
	...
	case 'external_link':
		$dimensions = function_exists('vc_extract_dimensions ')? vc_extract_dimensions( $external_img_size ) : false;
		...
		$img = array(
			'thumbnail' => '<img class="vc_single_image-img img-fluid" ' . $hwstring . ' src="' . $custom_src . '" alt="'.esc_attr($default_alt).'" />',
		);
		break;
...
if ( ! $img ) {
	$img['thumbnail'] = '<img class="vc_img-placeholder vc_single_image-img img-fluid" src="' . $default_src . '" alt="'.esc_attr($default_alt).'" />';
}
...
switch ( $onclick ) {
	...
	case 'video':
		$link = landpick_validate_video_url($video_link);
		break;	
