...
if( $style == 'landpick' ){
	$button_html = ( $btnarrow == 'yes' )? '<span>'. $title .' <i class="fa fa-angle-double-right"></i></span>': $title;	
}else{
	$button_html = $title;
}
...
if ( 'true' === $button_block && 'inline' !== $align ) {
	$button_classes[] = ( $style == 'landpick' )? 'btn-block' : 'vc_btn3-block';
}
if ( 'true' === $add_icon ) {
...
if ( 'custom' === $style ) {
	...
	$button_classes[] = ( $style == 'landpick' )? $color: 'vc_btn3-color-' . $color;
}
